...
const ControleSurVifAdd = () => {
  ...
  const addPpos = () => {
    setFormData(prev => ({
      ...prev,
      ppos: [...prev.ppos, { prNumber: prev.ppos.length + 1, type: '', designation: '', criteres: [{ reponse: '', coefficient: '1' }] }]
    }));
  };

  const removePpos = (index) => {
    setFormData(prev => {
      const newPpos = prev.ppos.filter((_, i) => i !== index);
      return { ...prev, ppos: newPpos };
    });
  };

  const handlePposChange = (index, field, value) => {
    setFormData(prev => {
      const newPpos = [...prev.ppos];
      newPpos[index][field] = value;
      return { ...prev, ppos: newPpos };
    });
  };

  const addPposCritere = (pposIndex) => {
    setFormData(prev => {
      const newPpos = [...prev.ppos];
      const lastCritere = newPpos[pposIndex].criteres[newPpos[pposIndex].criteres.length - 1];
      if (!lastCritere || (lastCritere.reponse !== '')) {
        newPpos[pposIndex].criteres = [...newPpos[pposIndex].criteres, { reponse: '', coefficient: '1' }];
      }
      return { ...prev, ppos: newPpos };
    });
  };

  const removePposCritere = (pposIndex, critIndex) => {
    setFormData(prev => {
      const newPpos = [...prev.ppos];
      newPpos[pposIndex].criteres = newPpos[pposIndex].criteres.filter((_, i) => i !== critIndex);
      return { ...prev, ppos: newPpos };
    });
  };

  const handlePposCritereChange = (pposIndex, critIndex, field, value) => {
    setFormData(prev => {
      const newPpos = [...prev.ppos];
      newPpos[pposIndex].criteres[critIndex][field] = value;
      return { ...prev, ppos: newPpos };
    });
  };
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    alert('Nouveau contrôle sur vif ajouté : ' + JSON.stringify(formData, null, 2));
    console.log('Nouveau contrôle sur vif ajouté : ' + JSON.stringify(formData, null, 2));
    navigate('/controle-sur-vif');
  };
  ...
  return (
    <div className="flex min-h-screen bg-gray-100">
      ...
      <div className="flex-grow flex flex-col">
        <Navbar page={"Nouveau PR/PPOS/DOC du controle sur le vif "}/>
        <main className="p-6 flex-grow overflow-auto max-w-7xl mx-auto">
          <h1 className="text-2xl font-bold mb-6">Ajouter un nouveau contrôle sur vif</h1>
          <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow p-6 space-y-8">
            ...
            {/* PPOS */}
            <div>
              <h2 className="text-xl font-semibold mb-4">Pratique Professionnels Observables en Situation (SEF/SST)</h2>
              {formData.ppos.map((pposItem, pposIndex) => (
                <div key={pposIndex} className="mb-6 p-4 border rounded relative bg-white shadow-sm">
                  <button
                    type="button"
                    onClick={() => removePpos(pposIndex)}
                    className="absolute top-2 right-2 text-red-600 hover:text-red-800"
                    title="Supprimer PPOS"
                  >
                    <Trash2 size={18} />
                  </button>
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div>
                      <label className="block mb-1 font-medium">N° PPOS</label>
                      <input
                        type="number"
                        value={pposItem.prNumber}
                        onChange={(e) => handlePposChange(pposIndex, 'prNumber', parseInt(e.target.value) || '')}
                        className="w-full px-3 py-2 border rounded"
                        min={1}
                        required
                      />
                    ...
                    <div>
                      <label className="block mb-1 font-medium">Type PPOS</label>
                      <select
                        value={pposItem.type}
                        onChange={(e) => handlePposChange(pposIndex, 'type', e.target.value)}
                        className="w-full px-3 py-2 border rounded"
                        required
                      >
                        <option value="">Sélectionnez un type</option>
                        {pposTypeOptions.map(type => (
                          <option key={type} value={type}>{type}</option>
                        ))}
                      </select>
                    </div>
                    <div className="md:col-span-2">
                      <label className="block mb-1 font-medium">Désignation PPOS</label>
                      <input
                        type="text"
                        value={pposItem.designation}
                        onChange={(e) => handlePposChange(pposIndex, 'designation', e.target.value)}
                        className="w-full px-3 py-2 border rounded"
                        required
                      />
                    </div>
                  ...
                  <div className="mt-4">
                    <label className="block mb-1 font-medium">Critères de contrôle</label>
                    {pposItem.criteres.map((crit, critIndex) => (
                      <div key={critIndex} className="flex items-center mb-2 gap-2">
                        <input
                          type="text"
                          value={crit.reponse}
                          onChange={(e) => handlePposCritereChange(pposIndex, critIndex, 'reponse', e.target.value)}
                          className="flex-grow px-3 py-2 border rounded"
                          required
                        />
                        <input
                          type="number"
                          value={crit.coeficient || 1}
                          onChange={(e) => handlePposCritereChange(pposIndex, critIndex, 'coeficient', e.target.value)}
                          className="w-20 px-3 py-2 border rounded"
                          min={1}
                          required
                          title="Coefficient"
                        ...
                        <button
                          type="button"
                          onClick={() => removePposCritere(pposIndex, critIndex)}
                          className="text-red-600 hover:text-red-800"
                          title="Supprimer critère"
                        >
                          <Trash2 size={16} />
                    ...
                    <button
                      type="button"
                      onClick={() => addPposCritere(pposIndex)}
                      className="mt-2 px-3 py-1 bg-green-600 text-white rounded hover:bg-green-700"
                    >
                      Ajouter un critère
                    </button>
...
export default ControleSurVifAdd;
